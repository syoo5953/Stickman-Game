...
public class EntitySpawnerImpl implements EntitySpawner {
  ...
  public Controllable createHero(ConfigurationProvider provider, Level level) {
    ...
    JSONObject data = provider.getLevelData(level.getLevelNum()).getEntityData();
  ...
  public Collection<Entity> createBackgroundEntities(ConfigurationProvider provider, Level level) {
    ...
    // create floor tiles
    for (int i = 0;
        i <= provider.getLevelData(level.getLevelNum()).getWidth() * 2;
        i += Constants.TILE_SIDE) {
      created.add(
          entityFactory.createEntity(
              "TILE", i, provider.getLevelData(level.getLevelNum()).getFloorHeight(), null));
    }

    // create flag
    if (ConfigurationUtils.containsValidJSONObject(
        provider.getLevelData(level.getLevelNum()).getEntityData(), "flag")) {
      created.add(
